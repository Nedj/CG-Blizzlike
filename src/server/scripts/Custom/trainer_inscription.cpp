        ...
        switch (uiAction) {
            case GOSSIP_ACTION_INFO_DEF + 1: // Learn Inscription
                ...
                if (pPlayer->HasSpell(45363) == true) {
                    ...
                    uint16 maxLevel = pPlayer->GetPureMaxSkillValue(SKILL_INSCRIPTION);
                    pPlayer->SetSkill(SKILL_INSCRIPTION, pPlayer->GetSkillStep(SKILL_INSCRIPTION), maxLevel, maxLevel);
                    ...
                    pCreature->MonsterWhisper("You have succesfully learned Inscription!", pPlayer->GetGUID());
                }
                break;
...
void AddSc_Script_TInscription() {
