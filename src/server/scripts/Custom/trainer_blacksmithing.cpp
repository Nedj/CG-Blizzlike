        ...
        switch (uiAction) {
            case GOSSIP_ACTION_INFO_DEF + 1: // Learn Blacksmithing
                ...
                if (pPlayer->HasSpell(51300) == true) {
                    ...
                    uint16 maxLevel = pPlayer->GetPureMaxSkillValue(SKILL_BLACKSMITHING);
                    pPlayer->SetSkill(SKILL_BLACKSMITHING, pPlayer->GetSkillStep(SKILL_BLACKSMITHING), maxLevel, maxLevel);
                    pPlayer->learnSpellHighRank(55628); // Socket Bracers
                    pPlayer->learnSpellHighRank(55641); // Socket Gloves 
                    pCreature->MonsterWhisper("You have succesfully learned Blacksmithing!", pPlayer->GetGUID());
                }
                break;
...
void AddSc_Script_TBlacksmithing() {
