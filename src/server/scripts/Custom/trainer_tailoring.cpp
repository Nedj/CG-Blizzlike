        ...
        switch (uiAction) {
            case GOSSIP_ACTION_INFO_DEF + 1: // Learn Tailoring
                ...
                if (pPlayer->HasSpell(51309) == true) {
                    ...
                    uint16 maxLevel = pPlayer->GetPureMaxSkillValue(SKILL_TAILORING);
                    pPlayer->SetSkill(SKILL_TAILORING, pPlayer->GetSkillStep(SKILL_TAILORING), maxLevel, maxLevel);
                    ...
                    pCreature->MonsterWhisper("You have succesfully learned Tailoring!", pPlayer->GetGUID());
                }
                break;
