        ...
        switch (uiAction) {
            case GOSSIP_ACTION_INFO_DEF + 1: // Learn Alchemy
                ...
                if (pPlayer->HasSpell(51304) == true) {
                    ...
                    uint16 maxLevel = pPlayer->GetPureMaxSkillValue(SKILL_ALCHEMY);
                    pPlayer->SetSkill(SKILL_ALCHEMY, pPlayer->GetSkillStep(SKILL_ALCHEMY), maxLevel, maxLevel);
                    pPlayer->learnSpellHighRank(53042); // Mixology
                    pPlayer->learnSpellHighRank(67025); // Flask of the North
                    pCreature->MonsterWhisper("You have succesfully learned Alchemy!", pPlayer->GetGUID());
                }
                break;
